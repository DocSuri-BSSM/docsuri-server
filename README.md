...
| `GEMINI_MODEL_REASONING` | 선택 | `gemini-3.1-pro-preview` | HS Code 추천·소명, 정정 요청서 등 추론 비중이 큰 호출. 무료 티어에서 쿼터가 막힐 수 있음 (`.env.example` 참고) |
| `GEMINI_MODEL_FAST` | 선택 | `gemini-3.6-flash` | 교차 검증 서술 등 빠른 응답이 필요한 호출 |
...
EC2 인스턴스 준비, 보안그룹, HTTPS 연결, 재배포/백업, 보안 체크리스트까지 전체 절차는 **[DEPLOYMENT.md](DEPLOYMENT.md)** 참고.
